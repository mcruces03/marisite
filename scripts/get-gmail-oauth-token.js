 ...
 * Run: npm run gmail-token   (or: node scripts/get-gmail-oauth-token.js)
...
const server = http.createServer(async (req, res) => {
  ...
  if (!code) {
    ...
    res.end(
      "<p>No se recibió código. Vuelve a ejecutar el script y autoriza de nuevo.</p>"
    );
    return;
  }
  try {
    ...
    res.end(
      "<p><strong>Listo.</strong> Ya tienes el refresh token. Puedes cerrar esta pestaña.</p>" +
        "<p>Cópialo en <code>.env.local</code> y en Vercel como <code>GMAIL_REFRESH_TOKEN</code></p>"
    );
    console.log("\n--- Añade esto en .env.local y Vercel ---\n");
